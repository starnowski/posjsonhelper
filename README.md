...
The RemoveJsonValuesFromJsonArrayFunction type is hibernate operator that invokes SQL function generated by the posjsonhelper library.
By default, generated function looks like on below example:

```sql
TE OR REPLACE FUNCTION {{schema}}.remove_values_from_json_array(input_json jsonb, values_to_remove jsonb) RETURNS jsonb AS $$
DECLARE
    result jsonb;
BEGIN
